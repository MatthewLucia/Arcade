...
bool Rect::isOverlapping(const Rect &r) const {
    ...
    if (r.getRightX() < this->getLeftX() || r.getLeftX() > this->getRightX()) {
        result = false;
    }
...
void Rect::draw() const {
    glColor4d(fill.red, fill.green, fill.blue, fill.alpha);
    glBegin(GL_QUADS);
    glVertex2d(center.x - (size.width / 2.0), center.y - (size.height / 2.0));
    glVertex2d(center.x - (size.width / 2.0), center.y + (size.height / 2.0));
    glVertex2d(center.x + (size.width / 2.0), center.y + (size.height / 2.0));
    glVertex2d(center.x + (size.width / 2.0), center.y - (size.height / 2.0));
    glEnd();
